...
function RealtimeSpider(){			
		...
		if(strpos($websrc,"key_block")==TRUE){		
			$prev_hash=$info->key_block->prev_hash;
			$prev_key_hash=$info->key_block->prev_key_hash;
			$block_height=$info->key_block->height;
		}
			
		if(strpos($websrc,"micro_block")==TRUE){
			$prev_hash= $info->micro_block->prev_hash;
			$prev_key_hash=$info->micro_block->prev_key_hash;
			$block_height=$info->micro_block->height;
			}			
		
		
		if($GLOBALS['lastheight']==$block_height){
			
			}else{
				$GLOBALS['txs']=" ";
				}
			
		$GLOBALS['lastheight']=$block_height;
		
		if($prev_hash!=$prev_key_hash){ProcessMicroBlock($prev_hash);}
		echo "Realtime=> top....checked.\n";
		//sleep(1);
...
function ProcessMicroBlock($microhash){
	...
	if(strpos($GLOBALS['txs'],$microhash)>0){
		
		}else{
			$GLOBALS['txs']=$GLOBALS['txs'].$microhash;
